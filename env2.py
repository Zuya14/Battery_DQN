...
class batteryEnv2(gym.Env):
    ...
    def __init__(self, lift_num, battery_num, working_minutes, max_limit_change=None, step_minutes=5):
        ...
        self.action_space = gym.spaces.Discrete(battery_num + 1) 
    ...
    def step(self, action):
        ...
        if action > 0:
            ...
            self.lifts[self.index] = self.batterys[action-1]
            self.batterys[action-1] = ex_batterys[self.index]
    ...
    def getState(self):
        # return np.concatenate([self.label, self.lifts, self.batterys, [self.sum_exchange], [self.left_time]])
        return np.concatenate([self.label, self.lifts/100.0, self.batterys/100.0, [self.old_action], [self.sum_exchange], [self.left_time/self.working_minutes]])
...
if __name__ == '__main__':
    ...
    env = batteryEnv2(10, 6, 550)
    env.reset(test=False)
    done = False

    env.reset(test=False)
    with open('env2_state.csv', 'w') as f:
        writer = csv.writer(f)
